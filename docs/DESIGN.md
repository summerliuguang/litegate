...
### M5 —— 可观测与运维（✅ 已完成 2026-09-08，后续批次增强至 v0.5.3）
- [x] 仪表盘增强：延迟 P50/P95（今日成功请求）、实时 RPM/TPM（最近 60 秒）、平均生成速度
      （输出 token/s，由 ttfb 推算生成时长）
- [x] 按密钥输出速度：密钥列表近 7 天加权平均 tok/s（`avg_tps`）+ 日志逐条 tok/s 列
- [x] 管理页移动端适配：宽表格手机端转卡片式（`data-label` + CSS，无 JS 框架）、
      统计卡响应式网格（移动两列）、弹窗触控优化；桌面布局不变
