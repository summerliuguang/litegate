...
func (s *Store) UpsertModelPrice(p *ModelPrice) error {
	...
	_, err := s.DB.Exec(
		`INSERT INTO model_prices(model, input_price, output_price, cache_read_price, currency, updated_at)
		 VALUES(?, ?, ?, ?, ?, datetime('now'))
		...
		p.Model, p.InputPrice, p.OutputPrice, p.CacheReadPrice, p.Currency)
	return err
}

func (s *Store) ListModelPrices() ([]ModelPrice, error) {
	rows, err := s.DB.Query(
		`SELECT model, input_price, output_price, cache_read_price, currency, updated_at FROM model_prices ORDER BY model`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var p ModelPrice
		if err := rows.Scan(&p.Model, &p.InputPrice, &p.OutputPrice, &p.CacheReadPrice, &p.Currency, &p.UpdatedAt); err != nil {
			return nil, err
		}
		out = append(out, p)
	}
