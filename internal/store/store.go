...
func migrate(db *sql.DB) error {
	...
	for _, m := range []struct{ table, col, ddl string }{
		...
		{"channels", "balance_api", `ALTER TABLE channels ADD COLUMN balance_api TEXT NOT NULL DEFAULT ''`},
		{"channels", "balance_alert_below", `ALTER TABLE channels ADD COLUMN balance_alert_below REAL NOT NULL DEFAULT 0`},
